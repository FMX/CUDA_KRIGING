...
void CMainFrame::ShowCudaStatus()
{
	if (((CWaferPainterApp*)AfxGetApp())->m_nocuda)
		if (!m_wndStatusBar.SetPaneText(2, "CUDA OFF!"))
		{
			AfxMessageBox("Update failed!");
		}
		/*this->GetMessageBar()->SetWindowTextA("CUDA SUPPORTED!");*/
	else
		if (!m_wndStatusBar.SetPaneText(2, "CUDA ON!"))
		{
			AfxMessageBox("Update failed!");
		}
	//this->GetMessageBar()->SetWindowTextA("CUDA NOT SUPPORTED!");
}
